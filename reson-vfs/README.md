...
`reson-vfs` is the optimized VFS storage layer shared by local/direct consumers and remote gateway consumers.
...
```text
migrations/postgres/001_reson_vfs_index.sql
```
...
- `reson_vfs_entries`
- `reson_vfs_packs`
- `reson_vfs_file_manifests`
- `reson_vfs_file_versions`
- `reson_vfs_mutations`
...
let pool = PgPool::connect("postgres://user:pass@localhost/reson").await?;
let index = Arc::new(PostgresVfsManifestIndex::new(pool));
let store = Arc::new(LocalObjectStoreClient::new(PathBuf::from(".run/reson-vfs-objects"))?);
...
```sh
cargo check --manifest-path reson-vfs/Cargo.toml \
...
```sh
export RESON_VFS_DATABASE_URL='postgres://user:pass@localhost/reson'
export RESON_VFS_OBJECT_ROOT='.run/reson-vfs-objects'
export RESON_VFS_SCOPE='dev-scope'
```
...
```rust
use reson_vfs::gcs_object_store::{GcsObjectStoreClient, GcsObjectStoreConfig};
...
Enable the `gateway` feature and use `GatewayVfsStorage` when storage is served through a Reson VFS HTTP gateway. It implements the same `OptimizedVfsStorage` trait, so callers can switch between local/direct and remote/gateway mode without changing call sites.
