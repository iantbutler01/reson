...
async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let database_url = env::var("RESON_VFS_DATABASE_URL")?;
    let object_root = env::var("RESON_VFS_OBJECT_ROOT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from(".run/reson-vfs-objects"));
    let scope = env::var("RESON_VFS_SCOPE").unwrap_or_else(|_| "dev-scope".to_string());
    ...
    vfs.write(
        "notes/hello.txt",
        Bytes::from_static(b"hello from reson-vfs"),
        None,
    )
    .await?;
