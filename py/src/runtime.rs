...
impl Runtime {
    ...
    fn tool(
        ...
        // Attach tool_type to function as __chevalier_tool_type__ attribute
        // (Python implementation uses this for schema generation)
        if let Some(ref tt) = tool_type {
            func.setattr(py, "__chevalier_tool_type__", tt)?;
        }
            ...
            Python::with_gil(|py| -> PyResult<PyObject> {
                let parsed_value = if let Some(text) = response.as_str() {
                    match serde_json::from_str::<serde_json::Value>(text) {
                        ...
                        Err(err) => {
                            eprintln!(
                                "py: failed to parse assistant text as JSON, returning string: {}",
                                err
                            );
