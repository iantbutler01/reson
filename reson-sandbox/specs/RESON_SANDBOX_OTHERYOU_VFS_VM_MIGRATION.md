<!-- @dive-file: Migration boundary for moving OtherYou VM/VFS runtime wiring into reson-sandbox. -->
<!-- @dive-rel: Complements RESON_SANDBOX_HA_DISTRIBUTED_CONTRACT.md by defining the product-facing API boundary. -->
<!-- @dive-rel: Source inventory is OtherYou api/src/services/runtime and api/src/services/computer/reson.rs. -->
# Reson Sandbox OtherYou VFS/VM Migration
...
Owner: Reson Sandbox
Intent: Make OtherYou consume a simple product-level public API for VM and VFS behavior, without owning vmd wiring, FUSE client/server transport, distributed placement machinery, CoreDNS/firewall details, or storage backend internals. The reson SDK surface is generic; product-specific names remain only inside product adapters and existing source-file references.
...
Reson should own runtime mechanics:
...
Any adapter that translates OtherYou product concepts into reson SDK requests lives in OtherYou, not reson.

Reson may expose generic SDK primitives such as owner refs, mount intents, backend contracts, checkpoint policy, ingress policy, and network policy. Reson must not contain OtherYou-specific adapter types, product-specific naming, product path semantics, or product database assumptions.

OtherYou owns the product adapter that maps its product concepts, user ownership, product paths, and product persistence into those generic reson primitives.

Current adapter location:

- OtherYou builds generic reson mount intents in `api/src/services/computer/reson.rs`.
- OtherYou emits generic vmd metadata keys: `reson.network_policy` and `reson.public_ingresses`.
- OtherYou accepts generic reson VFS/ingress headers and keeps legacy product-header compatibility only inside OtherYou routes.
- OtherYou local launch scripts export `RESON_SANDBOX_VFS_INTERNAL_SERVICE_TOKEN` and only enable guest network/CoreDNS defaults when explicitly requested or configured.
- Resolved gateway gap: `api/src/routes/runtime_nymfs_internal.rs` now owns only the OtherYou backend adapter and legacy aliasing. The generic HTTP FUSE server behavior, request DTOs, range parsing, lease/fence handshake, and status mapping live behind the reson `vfs-server` feature.

No adapter in reson is named for, shaped around, or coupled to the OtherYou product. Product-specific compatibility shims remain in OtherYou only.

## Target Public API Shape

The migration target is a reson-owned facade with a product-level shape similar to:

```rust
let runtime = reson_sandbox::vm::Runtime::new(config);
...
This is illustrative, not a final Rust API. The required shape is the important part: OtherYou passes product refs and intent, reson returns runtime surfaces and events.

The FUSE server side of the same API should have a product-adapter shape similar to:

```rust
let vfs_gateway = reson_sandbox_vfs_server::VfsGateway::new(
    VfsGatewayConfig {
...
let app = app.merge(vfs_gateway.routes("/v1/internal/reson/vfs/{owner_id}"));
```

This is also illustrative. The required boundary is:

- reson owns the generic HTTP/FUSE server contract and route behavior
...
Move a stable VM runtime facade into reson.
...
- OtherYou currently has the reson-specific provider in `api/src/services/computer/reson.rs`.
- OtherYou provider DTOs and trait live in `api/src/services/computer/mod.rs`.
- Reson already owns sandbox facade types in `reson-sandbox/crates/reson-sandbox/src/lib.rs`.

### 2) VFS Protocol and Router Contract

Move the VFS protocol contract into reson.
...
Current generic reson contract names:

- VFS headers: `x-reson-vfs-component`, `x-reson-vfs-surface-kind`, `x-reson-vfs-operation`, `x-reson-vfs-resource-key`, `x-reson-vfs-lock-owner-token`
...
- Public ingress owner headers: `x-reson-owner-bearer`, `x-reson-owner-cookie`, `x-reson-ingress-token`
- Public ingress token query parameter: `reson_ingress_token`
...
- Reson vmd client lives in `reson-sandbox/vmd/src/fuse/client.rs`.
- Reson FUSE implementation lives in `reson-sandbox/vmd/src/fuse/fs.rs`.

### 3) Generic FUSE Server / VFS Gateway

Move the generic HTTP server that backs vmd's FUSE client into reson.

This is distinct from moving durable storage mechanics. The server is the product-neutral gateway that accepts vmd FUSE requests, enforces the VFS protocol, parses ranges, acquires write leases, validates declared resource keys, maps errors to HTTP responses, and calls a product backend. The backend remains product-owned until reson also owns the durable storage metadata layer.

Preferred placement:

- `reson-sandbox/crates/reson-sandbox/src/vfs.rs` or equivalent for protocol constants, DTOs, and client/server-neutral contract types.
- A separate optional crate such as `reson-sandbox/crates/reson-sandbox-vfs-server`, or an opt-in `vfs-server` feature, for Axum/Tower server integration. The core SDK should not force Axum on consumers that only need the VM client.
...
- storage adapter implementations until durable VFS storage mechanics move into reson
- a compatibility alias for `/v1/internal/nymfs/{nym_id}` that delegates to the reson gateway while clients move to `/v1/internal/reson/vfs/{owner_id}`
...
The trait names are not prescribed. The invariant is that reson owns HTTP/FUSE correctness and OtherYou owns product semantics behind callbacks.

Cutover steps:

1. Add reson VFS protocol constants and DTOs that exactly match the current vmd client and OtherYou route behavior.
...
5. Mount the generic route under `/v1/internal/reson/vfs/{owner_id}` and keep `/v1/internal/nymfs/{nym_id}` as a product compatibility alias.
6. Update `api/src/services/computer/reson.rs` to build VFS endpoints through a generic reson helper instead of hard-coding `/internal/nymfs/{nym_id}`.
...
- some future durable storage adapter implementation behind the reson VFS backend trait
...
- generic reson VFS operation names and surface-kind headers
...
- Standard WebDAV/NFS/FUSE clients will not carry reson's operation, surface-kind, resource-key, and lock-owner headers without an adapter.
...
- Move the generic HTTP FUSE server into reson first.
...
- A rclone proof of concept is acceptable only if it sits behind the same reson VFS gateway and passes the same lease, ledger, range-read, write-flush, truncate, namespace-mutation, failover, and rewarm tests as the in-process backend.
...
- Our server boundary should stay. Rclone does not naturally carry the reson VFS headers, declared resource keys, action-counted owner tokens, product surface kind, or mutation ledger semantics. Replacing the gateway with WebDAV/NFS/rclone mount would reintroduce adapter glue at the wrong boundary.
...
- Add range-aware large-file caching in vmd FUSE.
  - Rclone's cache metadata tracks which byte ranges are present and fills missing ranges with downloader workers plus read-ahead. Our vmd FUSE cache is currently whole-file only for files under `LARGE_FILE_BYTES`; larger files fall through to independent HTTP range reads. A reson `RemoteFuseRangeCache` should cache byte windows per open file or per path, track present ranges, and serve repeated/sequential reads without round-tripping to the gateway for each FUSE read.
...
- Add explicit VFS cache observability.
  - Rclone exposes cache stats and writeback queue state through rc. Reson should expose counters for directory cache hits, small-file cache hits, large-range cache hits, backend range fetches, lease acquire latency, flush latency, dirty bytes, open handles, and eviction reason. This matters more than rclone replacement because optimization targets need production evidence.
...
- Do not replace the reson VFS protocol with WebDAV/NFS. Standard protocols do not carry reson's surface kind, operation, declared resource key, owner token, or product lease metadata.
- Do not use one shared cache root across distributed nodes. Rclone warns that cache eviction and in-use files are local concerns; distributed Reson nodes need per-node/per-owner cache isolation or explicit shared-cache coordination.
...
1. Move constants/DTOs into reson, then make vmd and the gateway share them.
...
Move shared mount planning into reson.
...
- OtherYou builds product VFS shared mounts in `api/src/services/computer/reson.rs`.
- OtherYou has mount constants and backend contract in `api/src/services/runtime/nym_fs_mount_contract.rs`.
- Reson shared mount DTO/proto lives in `reson-sandbox/crates/reson-sandbox/src/lib.rs` and `reson-sandbox/proto/bracket/vmd/v1/vmd.proto`.

### 7) Durable VFS Storage Mechanics

Move durable VFS storage mechanics into reson.
...
Move provider-level checkpoint and restore contract types into reson.
...
Move provider-facing ingress and network metadata contracts into reson.
...
- product API DTOs
- product audit logs
- deployment-specific external ingress reconciliation, until reson offers a generic deployment plugin for it
...
- Reson vmd public ingress proxy lives in `reson-sandbox/vmd/src/public_ingress.rs`.
- Reson firewall and DNS-adjacent runtime enforcement lives in `reson-sandbox/vmd/src/network/firewall.rs`.

## Keep In OtherYou

These are product-owned and should not migrate into reson:
...
This pass migrated the active OtherYou integration to the generic reson SDK boundary and validated the Linux runtime on corvidae. It did not complete the FUSE-server extraction; `api/src/routes/runtime_nymfs_internal.rs` still owns generic HTTP gateway behavior that must move into reson for the migration to be complete.

Reson changes:

- Added generic VM/VFS SDK helpers in `reson-sandbox/crates/reson-sandbox/src/vm.rs`.
- Exposed the generic module from `reson-sandbox/crates/reson-sandbox/src/lib.rs`.
...
- Made d2vm use the public converter image by default and require explicit `RESON_SANDBOX_D2VM_INCLUDE_BOOTSTRAP=true` for the legacy private bootstrap flag.
- Passed Docker API version through d2vm conversion via `RESON_SANDBOX_D2VM_DOCKER_API_VERSION` or `DOCKER_API_VERSION`.
- Added `RESON_SANDBOX_ENVOY_BASE_ID` so two test nodes on one Linux host can run Envoy without shared base-id socket conflicts.
- Fixed distributed ownership-fence defaults: when `RESON_SANDBOX_ETCD_PREFIX` is set and `RESON_SANDBOX_CONTROL_DEDUPE_PREFIX` is not, vmd now derives `<registry-prefix>/command-dedupe`, which makes control-bus fence reads use the same session-fence namespace as the SDK.
- Moved transient QMP and virtiofsd Unix sockets to `/tmp/reson-vmd/<vm-id>/...` by default, preserving VM data layout while avoiding Linux `sun_path` length failures from long data roots.

OtherYou changes:

- Kept the product adapter in `api/src/services/computer/reson.rs`.
...
- Accepted generic `x-reson-*` VFS and ingress headers in OtherYou routes while preserving legacy compatibility inside OtherYou. This first-pass route implementation was a temporary OtherYou-owned gateway; the extraction pass below replaces it with a reson-owned gateway plus an OtherYou backend adapter.
- Updated local VMD scripts so network services are opt-in instead of required for macOS local development.
- Updated OVH VMD deployment env to set `RESON_SANDBOX_D2VM_INCLUDE_BOOTSTRAP=true`, preserving the current private guest bootstrap contract while reson defaults remain generic.

## Completed Gateway Extraction Pass

This pass moves the generic HTTP/FUSE gateway behavior into reson while keeping the product adapter in OtherYou.

Reson changes:

- Added `reson-sandbox/crates/reson-sandbox/src/vfs.rs` with generic VFS protocol headers, DTOs, endpoint helpers, scope-path joining, range parsing, gateway errors, and an opt-in `vfs-server` Axum route builder.
- Added `reson-sandbox` feature `vfs-server`, keeping Axum and async-trait out of the default VM client surface unless a consumer needs the HTTP gateway.
- Added `reson-rust` feature `sandbox-vfs-server` so consumers can access the gateway through `reson_agentic::sandbox`.
- Updated vmd's FUSE client/cache/fs code to use the shared reson VFS DTOs, header constants, operation names, surface-kind constants, and scope path helper.
- Kept the vmd FUSE server/client generic: no OtherYou, product, or legacy `x-nymfs-*` names exist in the new reson VFS module or vmd FUSE protocol code.
...
- Mounted the generic reson route at `/v1/internal/reson/vfs/{owner_id}`.
- Kept `/v1/internal/nymfs/{nym_id}` as a compatibility alias using the same reson gateway route builder and the same OtherYou backend.
...
- Updated `api/src/services/computer/reson.rs` to build new VFS endpoints through `reson_agentic::sandbox::vfs::owner_vfs_endpoint`, so new VM mounts target `/v1/internal/reson/vfs/{owner_id}` instead of hard-coding the legacy product route.
...
- `rustup run 1.90-aarch64-apple-darwin cargo test --manifest-path api/Cargo.toml services::computer::reson::tests::build_nymfs_shared_mounts_uses_shared_storage_contract_for_gcs`
...
This validation used the moved generic reson VFS gateway in OtherYou, not the old product-owned gateway implementation.

Setup:

- Local OtherYou API served product routes on `127.0.0.1:3002`.
- Reverse SSH forwarded corvidae's service callback `127.0.0.1:3001` to the local API, so VMD FUSE callbacks hit `/v1/internal/reson/vfs/{owner_id}`.
- Corvidae VMD nodes ran the rebuilt reson image with GCS VFS support, host networking, KVM, FUSE, CoreDNS, Envoy, etcd, and NATS.
- Test bucket: `reson-otheryou-vfs-vm-migration-20260605-b0f78571`.
...
- New mounts used the generic reson VFS endpoint shape: `/v1/internal/reson/vfs/{owner_id}`.
...
- `nym_computer_exec` ran through OtherYou's product runtime to the corvidae VM and wrote through FUSE:
  ...
  - result: exit code `0`
- Generic VFS stat through `/v1/internal/reson/vfs/{owner_id}/stat?path=.runtime/current-task-workspace/agent-vfs-patched-dd.bin` returned a file with `size_bytes: 16`.
...
- Distributed exec fence mismatch:
  ...
  - Cause: SDK wrote `/reson-sandbox-otheryou/session_fences/...`; vmd read fences through the default `/reson-sandbox/command-dedupe`-derived namespace.
  - Fix: derive the control dedupe prefix from `RESON_SANDBOX_ETCD_PREFIX` when an explicit control dedupe prefix is absent.

- Multi-node Envoy on one Linux host:
  - Symptom: the second node's Envoy could fail on shared base-id runtime sockets.
  - Fix: added `RESON_SANDBOX_ENVOY_BASE_ID` and pass `--base-id` to Envoy.

- Long QMP socket path:
  - Symptom: node2 failed to boot a VM with `UNIX socket path ... is too long` for `<long-data-dir>/<uuid>/qmp.sock`.
  - Fix: store transient QMP and virtiofsd sockets under `/tmp/reson-vmd/<vm-id>/...`, with runtime-dir ownership prepared for the configured QEMU uid/gid.
...
- OVH private d2vm bootstrap:
  ...
  - Fix: reson defaults do not pass the private flag, and OtherYou OVH explicitly opts in with `RESON_SANDBOX_D2VM_INCLUDE_BOOTSTRAP=true`.
...
- [x] The spec includes cutover steps for reson, vmd, and OtherYou.
...
- [x] The spec evaluates keeping the reson gateway versus replacing it with rclone and records the decision.

## First Interface Pass Acceptance Criteria

- [x] OtherYou can request VM sessions through the generic reson SDK boundary without owning distributed control wiring.
- [x] OtherYou adapter code lives in OtherYou.
- [x] Reson generic SDK code has no OtherYou-specific adapter naming.
- [x] OtherYou accepts generic reson VFS/ingress headers and keeps legacy product compatibility inside OtherYou.
...
- [x] Distributed deployment remains available through reson configuration, with OtherYou selecting endpoints/profile rather than building control-plane machinery.
...
- [x] Reson owns the generic HTTP FUSE server/VFS gateway instead of OtherYou route code owning it.
- [x] vmd's `RemoteVfsClient` and the reson gateway share one protocol/DTO/constant source where practical.
- [x] OtherYou implements a product backend adapter for the reson gateway.
- [x] OtherYou's `/v1/internal/nymfs/{nym_id}` path is only a compatibility alias that delegates to the generic reson route.
- [x] OtherYou's reson computer provider builds VFS endpoints through a generic reson helper, not by hard-coding the product route shape.
...
- [x] No rclone-backed proof of concept was selected for this migration. Any future rclone-backed implementation must sit behind the reson gateway and pass the same gateway contract tests before it can replace the in-process backend.

Longer-term extraction still allowed by this spec, but not required for the FUSE-server move:

- Move durable VFS object-store pack/cache/compaction internals from OtherYou into reson once reson owns a generic storage metadata backend.
- Move product-visible checkpoint manifest storage into reson once product checkpoint rows can reference a generic manifest identity.

## Non-Goals

- Do not move product models into reson.
- Do not make reson depend on OtherYou DB schemas.
...
- `rustup run 1.90-aarch64-apple-darwin cargo test -p reson-sandbox vm:: -- --nocapture`
- `rustup run 1.90-aarch64-apple-darwin cargo test -p reson-sandbox session_create_forwards_shared_mounts_contract -- --nocapture`
...
- `rustup run 1.90-aarch64-apple-darwin cargo test --manifest-path api/Cargo.toml services::computer::reson -- --nocapture`
...
- Built `reson-vmd:otheryou-migration-test` from the updated reson sandbox with public d2vm, Envoy, and CoreDNS images.
...
- The current private OtherYou guest image still depends on the legacy d2vm bootstrap flag. That dependency is now explicit in OtherYou OVH config instead of being hidden in reson defaults.
