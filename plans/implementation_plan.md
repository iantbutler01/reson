...
**Modified files:**
- `chevalier/services/inference_clients.py` - Add ToolCall class with factory methods and provider conversion
- `chevalier/types.py` - Export ToolCall in __all__ list and import statement
- `chevalier/chevalier.py` - Update type hints to accept ToolCall in history parameters
- `chevalier/services/inference_clients.py` - Update `_convert_messages_to_provider_format` method to handle ToolCall objects
- `chevalier/services/inference_clients.py` - Update `GoogleGenAIInferenceClient._build_parts` to support functionCall parts
...
**New functions:**
- `ToolCall.create(tool_call_obj_or_list, signature=None)` in `chevalier/services/inference_clients.py` - Factory method for creating ToolCall from provider formats
- `ToolCall._create_single(tool_call_obj, signature=None)` in `chevalier/services/inference_clients.py` - Internal method to create single ToolCall from various formats
- `ToolCall.to_provider_assistant_message(provider)` in `chevalier/services/inference_clients.py` - Convert ToolCall to provider-specific assistant message format

**Modified functions:**
- `InferenceClient._convert_messages_to_provider_format()` in `chevalier/services/inference_clients.py` - Add ToolCall handling to message conversion logic
- `GoogleGenAIInferenceClient._build_parts()` in `chevalier/services/inference_clients.py` - Add support for functionCall content in parts building

## [Classes]
Add ToolCall class as a new conversation history element type.

**New classes:**
- `ToolCall(ChevalierBase)` in `chevalier/services/inference_clients.py` - Main class for representing assistant tool calls with provider conversion methods

**Modified classes:**
- Update type hints in `Runtime` class methods (`run`, `run_stream`) in `chevalier/chevalier.py` to accept `ToolCall` in history parameters
...
1. **Add ToolCall class definition** - Create the base ToolCall class with fields and basic structure in `chevalier/services/inference_clients.py`
...
6. **Export in types module** - Add ToolCall import and export in `chevalier/types.py`

7. **Update type hints** - Modify Runtime method signatures in `chevalier/chevalier.py` to accept ToolCall in history parameters
