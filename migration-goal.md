• Completion Goal
  ...
  Finish the Chevalier/OtherYou VFS+VMD migration to a product-ready state where OtherYou consumes the generic Chevalier VM/VFS gateway boundary, the GCS-backed VFS path is verified end-
  ...
  Notably the migration is creating a shared higher levelish interface for these in chevalier as we do intend shortly to have multiple VM providers.
  ...
  1. Branches are coherent and clean:
      ...
      - both clean, committed, rebased, and OtherYou compiles against sibling Chevalier.
  ...
  3. Chevalier gates pass:
      - cargo test -p chevalier-sandbox --features vfs-server vfs::
      - rustup run 1.90-aarch64-apple-darwin cargo test -p vmd fuse::
      - shared mount/facade tests pass.
      - Chevalier VFS/FUSE code remains product-neutral: no OtherYou/Nym semantics in generic Chevalier modules.
  ...
  4. OtherYou adapter gates pass:
      ...
      - new /internal/chevalier/vfs/{owner_id} route works.
      - new VM mounts use /v1/internal/chevalier/vfs/{owner_id}.
  ...
  6. GCS-backed VFS path is proven clean:
      ...
      - stat/read through /v1/internal/chevalier/vfs/{owner_id}.
  ...
  - identify required Chevalier + OtherYou image/build steps.
  - record known smoke checks for after deploy.
  - do not mutate prod until you explicitly approve after morning verification.

  Definition Of Done

  The migration is complete when the branch pair compiles, Chevalier and OtherYou tests pass, the GCS-backed generic VFS route is proven through a real corvidae VM lifecycle, cleanup
