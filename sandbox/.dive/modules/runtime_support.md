...
## Files
- `crates/sandbox/build.rs` - Facade protobuf generation wiring for vmd/portproxy contracts.
- `crates/sandbox/src/slo.rs` - SLO budget model and evaluator used by rollout/gate policy checks.
...
## Relationships
- `crates/sandbox/build.rs` -> `crates/sandbox/src/lib.rs`: generated protobuf types feed facade API transport calls.
- `crates/sandbox/src/slo.rs` -> `scripts/verify_slo_profile.sh`: SLO evaluator results drive gate and rollout policy checks.
