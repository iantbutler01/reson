    ...
    let (nats_url, nats_auth_token, nats_subject_prefix, node_id) =
        if let Some(control) = control_bus {
            ...
            (
                None,
                None,
                "chevalier.sandbox.control".to_string(),
                node_registry.node_id.clone(),
            )
        };
...
mod tests {
    ...
    fn reconcile_plan_upserts_missing_and_deletes_stale() {
        ...
        let routes = vec![
            SessionRouteEntry {
                key: "/sandbox/sessions/s1".to_string(),
                route: SessionRoute {
            ...
            SessionRouteEntry {
                key: "/sandbox/sessions/s_stale".to_string(),
                route: SessionRoute {
        ...
        assert_eq!(
            plan.delete_keys,
            vec![
                "/sandbox/sessions/s1".to_string(),
                "/sandbox/sessions/s_stale".to_string(),
                session_fence_key("/chevalier-sandbox", 16, "s_stale"),
                legacy_session_fence_key("/chevalier-sandbox", "s_stale"),
            ]
        );
    ...
    fn reconcile_plan_preserves_fence_when_session_has_nonlocal_route() {
        ...
        let routes = vec![
            SessionRouteEntry {
                key: "/sandbox/sessions/01/s1".to_string(),
                route: SessionRoute {
                    session_id: "s1".to_string(),
            ...
            SessionRouteEntry {
                key: "/sandbox/sessions/02/s1".to_string(),
                route: SessionRoute {
                    session_id: "s1".to_string(),
        ...
        assert_eq!(
            plan.delete_keys,
            vec!["/sandbox/sessions/01/s1".to_string()]
        );
    }

    #[test]
    fn sharded_session_key_detection_is_precise() {
        let prefix = "/sandbox/sessions/";
        assert!(is_sharded_session_key(
            prefix,
            "/sandbox/sessions/03/session-a"
        ));
        assert!(is_sharded_session_key(
            prefix,
            "/sandbox/sessions/15/session-b"
        ));
        assert!(!is_sharded_session_key(
            prefix,
            "/sandbox/sessions/session-legacy"
        ));
        assert!(!is_sharded_session_key(
            prefix,
            "/sandbox/sessions/ab/session-c"
        ));
    }
}
