...
Sandbox runtime subproject for Chevalier.

This subproject is the extraction target for Rust VM sandbox runtime components currently implemented in OpenBracket (`vmd`, `portproxy`, and their proto contracts). It provides a stable dependency boundary for `rust` via the high-level `chevalier-sandbox` facade crate.
...
- Real-gate CI requirements and profile capabilities are documented in `specs/CHEVALIER_SANDBOX_REAL_CI_REQUIREMENTS.md`.
...
- Distributed control is enabled by default (feature `distributed-control`):
  - etcd stores node registry + durable `session_id -> node endpoint` bindings under `/chevalier-sandbox`.
  ...
  - `vmd` can self-register with etcd lease heartbeats when `CHEVALIER_SANDBOX_ETCD_ENDPOINTS` is set.
  ...
  - HA target contract and remaining scope are tracked in `specs/CHEVALIER_SANDBOX_HA_DISTRIBUTED_CONTRACT.md`.
- Security profile (production-facing):
  - `vmd` supports bearer-token auth via `--auth-token` / `CHEVALIER_SANDBOX_AUTH_TOKEN`.
  - Optional read-only token is supported via `--readonly-auth-token` / `CHEVALIER_SANDBOX_READONLY_AUTH_TOKEN`.
...
- Admission control baseline:
  - `vmd` supports `--max-active-vms` (or `CHEVALIER_SANDBOX_MAX_ACTIVE_VMS`) to reject over-capacity session creates with explicit `resource_exhausted` errors.
- Storage profile and HA mode baseline:
  - `vmd` supports `--storage-profile {local-ephemeral|durable-shared}` (or `CHEVALIER_SANDBOX_STORAGE_PROFILE`).
  - `vmd --ha-mode` (or `CHEVALIER_SANDBOX_HA_MODE=1`) enforces durable-shared storage and requires both etcd node registry + NATS control bus configuration.
  - Continuity tier policy is enforced: distributed HA defaults to Tier-B (`CHEVALIER_SANDBOX_CONTINUITY_TIER=tier-b`), and Tier-A is permitted only with explicit degraded mode (`CHEVALIER_SANDBOX_DEGRADED_MODE=1`).
  ...
  - Fork lineage metadata includes storage profile and fork durability hints (`chevalier.storage_profile`, `chevalier.fork_durability_class`, `chevalier.fork_restore_scope`).
- SLO profile:
  - Threshold schema is defined in `specs/CHEVALIER_SANDBOX_SLO_THRESHOLDS.json`.
  - Local strict verification enforces schema + instrumentation + evaluator tests.
  - CI/nightly can enforce observed SLO budgets by setting `CHEVALIER_SANDBOX_SLO_OBSERVED_FILE`.
  - CI/nightly can enforce staged rollout pause/rollback decisions via `CHEVALIER_SANDBOX_ROLLOUT_OBSERVED_FILE` and `scripts/evaluate_rollout_policy.sh`.
...
- Envelope compatibility baseline:
  ...
  - Compatibility policy is documented in `specs/CHEVALIER_SANDBOX_ENVELOPE_COMPATIBILITY_POLICY.md`.
