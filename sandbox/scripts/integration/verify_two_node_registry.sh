...
# @dive-rel: Implements checklist item 7.2.1 from specs/CHEVALIER_SANDBOX_REAL_INTEGRATION_TEST_PLAN.md with machine-verifiable etcd evidence.
set -euo pipefail
...
NODE1_ID="${CHEVALIER_SANDBOX_INTEGRATION_NODE1_ID:-integration-node-1}"
NODE2_ID="${CHEVALIER_SANDBOX_INTEGRATION_NODE2_ID:-integration-node-2}"
NODE1_PORT="${CHEVALIER_SANDBOX_INTEGRATION_NODE1_PORT:-19072}"
NODE2_PORT="${CHEVALIER_SANDBOX_INTEGRATION_NODE2_PORT:-19073}"
NODE1_ADMISSION_FROZEN="${CHEVALIER_SANDBOX_INTEGRATION_NODE1_ADMISSION_FROZEN:-$RUN_DRAIN_TEST}"
NODE2_ADMISSION_FROZEN="${CHEVALIER_SANDBOX_INTEGRATION_NODE2_ADMISSION_FROZEN:-0}"
HEARTBEAT_OBSERVE_SECS="${CHEVALIER_SANDBOX_INTEGRATION_HEARTBEAT_OBSERVE_SECS:-3}"
REGISTRY_TTL_SECS="${CHEVALIER_SANDBOX_INTEGRATION_REGISTRY_TTL_SECS:-4}"
READY_TIMEOUT_SECS="${CHEVALIER_SANDBOX_INTEGRATION_NODE_READY_TIMEOUT_SECS:-60}"
...
if [[ "$RUN_FACADE_TEST" -eq 1 ]]; then
  ...
  (
    cd "$REPO_ROOT"
    CHEVALIER_SANDBOX_REAL_PRIMARY_ENDPOINT="http://127.0.0.1:19079" \
    CHEVALIER_SANDBOX_REAL_GATEWAY_ENDPOINTS="${node1_server},${node2_server}" \
      cargo test \
        -p chevalier-sandbox \
...
if [[ "$RUN_DRAIN_TEST" -eq 1 ]]; then
  ...
  (
    cd "$REPO_ROOT"
    CHEVALIER_SANDBOX_REAL_PRIMARY_ENDPOINT="$node1_server" \
    CHEVALIER_SANDBOX_REAL_SECONDARY_ENDPOINT="$node2_server" \
    ...
    CHEVALIER_SANDBOX_REAL_NATS_SUBJECT_PREFIX="chevalier.sandbox.control.integration.registry" \
    CHEVALIER_SANDBOX_REAL_NATS_STREAM="CHEVALIER_SANDBOX_CONTROL_REGISTRY" \
      cargo test \
        -p chevalier-sandbox \
