...
# @dive-rel: Executes crates/sandbox/tests/real_control_plane_failures.rs against live compose-backed etcd+nats services.
...
NODE1_ID="${CHEVALIER_SANDBOX_INTEGRATION_CP_NODE1_ID:-integration-control-node-1}"
NODE1_PORT="${CHEVALIER_SANDBOX_INTEGRATION_CP_NODE1_PORT:-19184}"
REGISTRY_TTL_SECS="${CHEVALIER_SANDBOX_INTEGRATION_CP_REGISTRY_TTL_SECS:-6}"
READY_TIMEOUT_SECS="${CHEVALIER_SANDBOX_INTEGRATION_CP_READY_TIMEOUT_SECS:-90}"
CONTROL_SUBJECT_PREFIX="${CHEVALIER_SANDBOX_INTEGRATION_CONTROL_SUBJECT_PREFIX:-chevalier.sandbox.control.integration.failures}"
CONTROL_STREAM_NAME="${CHEVALIER_SANDBOX_INTEGRATION_CONTROL_STREAM_NAME:-CHEVALIER_SANDBOX_CONTROL_FAILURES}"
...
run_real_tests() {
  ...
  for selector in "${selectors[@]}"; do
    ...
    (
      cd "$REPO_ROOT"
      RUST_TEST_THREADS=1 \
      CHEVALIER_SANDBOX_REAL_PRIMARY_ENDPOINT="$endpoint" \
      ...
      CHEVALIER_SANDBOX_REAL_ETCD_DEGRADE_CONTAINERS="$etcd2_id,$etcd3_id" \
      CHEVALIER_SANDBOX_REAL_NATS_CONTAINER_ID="$nats_id" \
        cargo test \
          -p chevalier-sandbox \
...
run_reconcile_convergence_after_restart() {
  ...
  (
    ...
    CHEVALIER_SANDBOX_REAL_NATS_SUBJECT_PREFIX="$CONTROL_SUBJECT_PREFIX" \
    CHEVALIER_SANDBOX_REAL_NATS_STREAM="$CONTROL_STREAM_NAME" \
    CHEVALIER_SANDBOX_REAL_RECONCILE_CONVERGENCE_TIMEOUT_SECS="${CHEVALIER_SANDBOX_INTEGRATION_RECONCILE_CONVERGENCE_TIMEOUT_SECS:-30}" \
      cargo test \
        -p chevalier-sandbox \
        --test real_control_plane_failures \
