...
# @dive-rel: Executes crates/sandbox/tests/real_warm_pool_pipeline.rs selectors for section 7.4 checklist coverage.
set -euo pipefail
...
NODE1_ID="${CHEVALIER_SANDBOX_INTEGRATION_WARM_NODE1_ID:-integration-warm-node-1}"
NODE1_PORT="${CHEVALIER_SANDBOX_INTEGRATION_WARM_NODE1_PORT:-19186}"
REGISTRY_TTL_SECS="${CHEVALIER_SANDBOX_INTEGRATION_WARM_REGISTRY_TTL_SECS:-6}"
READY_TIMEOUT_SECS="${CHEVALIER_SANDBOX_INTEGRATION_WARM_READY_TIMEOUT_SECS:-90}"
CONTROL_SUBJECT_PREFIX="${CHEVALIER_SANDBOX_INTEGRATION_WARM_SUBJECT_PREFIX:-chevalier.sandbox.control.integration.warm}"
CONTROL_STREAM_NAME="${CHEVALIER_SANDBOX_INTEGRATION_WARM_STREAM_NAME:-CHEVALIER_SANDBOX_CONTROL_WARM}"
...
for selector in "${selectors[@]}"; do
  (
    ...
    CHEVALIER_SANDBOX_REAL_WARM_POOL_DATA_DIR="$DATA_DIR" \
    CHEVALIER_SANDBOX_REAL_VMD_BIN="$VMD_BIN" \
      cargo test \
        -p chevalier-sandbox \
