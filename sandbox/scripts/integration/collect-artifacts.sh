...
integration_log "collecting integration artifacts (profile=${CHEVALIER_SANDBOX_INTEGRATION_PROFILE})"
...
env | grep '^CHEVALIER_SANDBOX_INTEGRATION_' | sort >"$profile_env_file" 2>&1 || true
...
compose_file_rel="${CHEVALIER_SANDBOX_INTEGRATION_COMPOSE_FILE}"

cat >"$manifest_file" <<EOF_MANIFEST
{
  "schema": "chevalier.sandbox.integration.artifact_bundle.v1",
  "generated_at_utc": "$generated_at",
  "profile": "${CHEVALIER_SANDBOX_INTEGRATION_PROFILE}",
  "reason": "$REASON",
  "project": "${CHEVALIER_SANDBOX_INTEGRATION_PROJECT}",
  "compose_file": "$compose_file_rel",
  "control_plane": {
    "etcd_endpoints": "${CHEVALIER_SANDBOX_INTEGRATION_ETCD_ENDPOINTS}",
    "nats_url": "${CHEVALIER_SANDBOX_INTEGRATION_NATS_URL}"
  },
  "artifacts": {
