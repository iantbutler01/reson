...
# @dive-rel: Executes crates/sandbox/tests/real_failover_continuity.rs to satisfy the 7.3 continuity/stream-resume/no-rerun checklist items.
set -euo pipefail
...
NODE_PREFIX="${CHEVALIER_SANDBOX_INTEGRATION_NODE_KEY_PREFIX:-/chevalier-sandbox}"
NODE_PREFIX="${NODE_PREFIX%/}"
if [[ -z "$NODE_PREFIX" ]]; then
  NODE_PREFIX="/chevalier-sandbox"
fi

NODE1_ID="${CHEVALIER_SANDBOX_INTEGRATION_FAILOVER_NODE1_ID:-integration-failover-node-1}"
NODE2_ID="${CHEVALIER_SANDBOX_INTEGRATION_FAILOVER_NODE2_ID:-integration-failover-node-2}"
NODE1_PORT="${CHEVALIER_SANDBOX_INTEGRATION_FAILOVER_NODE1_PORT:-19182}"
NODE2_PORT="${CHEVALIER_SANDBOX_INTEGRATION_FAILOVER_NODE2_PORT:-19183}"
REGISTRY_TTL_SECS="${CHEVALIER_SANDBOX_INTEGRATION_FAILOVER_REGISTRY_TTL_SECS:-6}"
READY_TIMEOUT_SECS="${CHEVALIER_SANDBOX_INTEGRATION_FAILOVER_READY_TIMEOUT_SECS:-90}"
VM_SOURCE_REF="${CHEVALIER_SANDBOX_INTEGRATION_VM_SOURCE_REF:-${BRACKET_VM_IMAGE:-ghcr.io/bracketdevelopers/uv-builder:main}}"
CONTROL_SUBJECT_PREFIX="${CHEVALIER_SANDBOX_INTEGRATION_CONTROL_SUBJECT_PREFIX:-chevalier.sandbox.control.integration.failover}"
CONTROL_STREAM_NAME="${CHEVALIER_SANDBOX_INTEGRATION_CONTROL_STREAM_NAME:-CHEVALIER_SANDBOX_CONTROL_FAILOVER}"
...
FORCE_STOP_BEFORE_ATTACH="${CHEVALIER_SANDBOX_REAL_FORCE_STOP_BEFORE_ATTACH:-1}"
...
start_nodes() {
  ...
  if [[ "$DISTRIBUTED_MODE" -eq 1 ]]; then
    CHEVALIER_SANDBOX_ETCD_ENDPOINTS="$CHEVALIER_SANDBOX_INTEGRATION_ETCD_ENDPOINTS" \
    CHEVALIER_SANDBOX_NATS_URL="$CHEVALIER_SANDBOX_INTEGRATION_NATS_URL" \
    CHEVALIER_SANDBOX_NATS_SUBJECT_PREFIX="$CONTROL_SUBJECT_PREFIX" \
    CHEVALIER_SANDBOX_NATS_STREAM="$CONTROL_STREAM_NAME" \
      "$VMD_BIN" \
      --listen "127.0.0.1:${NODE1_PORT}" \
      --data-dir "$SHARED_DATA_DIR" \
      --node-id "$NODE1_ID" \
      --advertise-endpoint "$node1_endpoint" \
      --registry-etcd-endpoints "$CHEVALIER_SANDBOX_INTEGRATION_ETCD_ENDPOINTS" \
      --registry-prefix "$NODE_PREFIX" \
      --registry-ttl-secs "$REGISTRY_TTL_SECS" \
      --control-nats-url "$CHEVALIER_SANDBOX_INTEGRATION_NATS_URL" \
...
run_real_test() {
  ...
  if [[ "$DISTRIBUTED_MODE" -eq 1 ]]; then
    (
      ...
      CHEVALIER_SANDBOX_REAL_PRIMARY_ENDPOINT="$primary_endpoint" \
      CHEVALIER_SANDBOX_REAL_SECONDARY_ENDPOINT="$secondary_endpoint" \
      CHEVALIER_SANDBOX_REAL_PRIMARY_PID="$NODE1_PID" \
      CHEVALIER_SANDBOX_REAL_FORCE_STOP_BEFORE_ATTACH="$FORCE_STOP_BEFORE_ATTACH" \
      CHEVALIER_SANDBOX_REAL_ETCD_ENDPOINTS="$CHEVALIER_SANDBOX_INTEGRATION_ETCD_ENDPOINTS" \
      CHEVALIER_SANDBOX_REAL_ETCD_PREFIX="$NODE_PREFIX" \
      CHEVALIER_SANDBOX_REAL_NATS_URL="$CHEVALIER_SANDBOX_INTEGRATION_NATS_URL" \
      CHEVALIER_SANDBOX_REAL_NATS_SUBJECT_PREFIX="$CONTROL_SUBJECT_PREFIX" \
      CHEVALIER_SANDBOX_REAL_NATS_STREAM="$CONTROL_STREAM_NAME" \
        cargo test \
          -p chevalier-sandbox \
...
integration_log "starting shared integration control plane (profile=${CHEVALIER_SANDBOX_INTEGRATION_PROFILE})"
