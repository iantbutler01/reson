...
STRICT=0
THRESHOLDS_FILE="$REPO_ROOT/specs/CHEVALIER_SANDBOX_SLO_THRESHOLDS.json"
OBSERVED_FILE="${CHEVALIER_SANDBOX_SLO_OBSERVED_FILE:-}"
ROLLOUT_OBSERVED_FILE="${CHEVALIER_SANDBOX_ROLLOUT_OBSERVED_FILE:-}"
...
rg -n "log_slo_observation\\(\"session\\.create\"" "$REPO_ROOT/crates/sandbox/src/lib.rs" >/dev/null
rg -n "log_slo_observation\\(\"session\\.attach\"" "$REPO_ROOT/crates/sandbox/src/lib.rs" >/dev/null
rg -n "log_slo_observation\\(\"exec\\.stream\\.establish\\.warm_vm\"" "$REPO_ROOT/crates/sandbox/src/lib.rs" >/dev/null

log "slo gate: budget evaluator tests"
(cd "$REPO_ROOT" && cargo test -p chevalier-sandbox slo_budget_ -- --nocapture)

log "slo gate: rollout policy evaluator contract tests"
tmp_dir="$(mktemp -d "/tmp/chevalier_sandbox_slo_gate.XXXXXX")"
...
if [[ -n "$OBSERVED_FILE" ]]; then
  ...
  warn "CHEVALIER_SANDBOX_SLO_OBSERVED_FILE is unset; observed threshold enforcement skipped (expected in local strict runs)"
fi
...
if [[ -n "$ROLLOUT_OBSERVED_FILE" ]]; then
  ...
  warn "CHEVALIER_SANDBOX_ROLLOUT_OBSERVED_FILE is unset; rollout pause/rollback enforcement skipped (expected in local strict runs)"
fi
