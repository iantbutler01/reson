...
out_dir=""
server_cn="${CHEVALIER_SANDBOX_TLS_SERVER_CN:-chevalier-sandbox-server}"
client_cn="${CHEVALIER_SANDBOX_TLS_CLIENT_CN:-chevalier-sandbox-client}"
days="${CHEVALIER_SANDBOX_TLS_DAYS:-365}"
manifest=""
...
openssl req -x509 -new -nodes -key "$ca_key" -sha256 -days "$days" -subj "/CN=chevalier-sandbox-ca" -out "$ca_cert" >/dev/null 2>&1
