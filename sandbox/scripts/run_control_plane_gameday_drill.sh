...
etcd_port="${CHEVALIER_SANDBOX_GAMEDAY_ETCD_PORT:-32379}"
nats_port="${CHEVALIER_SANDBOX_GAMEDAY_NATS_PORT:-14222}"
nats_monitor_port="${CHEVALIER_SANDBOX_GAMEDAY_NATS_MONITOR_PORT:-18222}"
project_name="chevalier-sandbox-gameday-$$"
...
if [[ -z "$OUTPUT" ]]; then
  OUTPUT="/tmp/chevalier_sandbox_control_plane_gameday_${timestamp}.txt"
fi
...
set +e
etcdctl_cluster put /sandbox/gameday/baseline ok >/dev/null
rc=$?
set -e
if [[ "$rc" -ne 0 ]]; then
  etcd_status="failed"
  ...
  set +e
  etcdctl_single --command-timeout=5s put /sandbox/gameday/quorum-loss expected-fail >/dev/null
  rc=$?
  set -e
  if [[ "$rc" -eq 0 ]]; then
    etcd_status="failed"
    ...
    if ! wait_for_etcd_cluster; then
      ...
      set +e
      etcdctl_cluster put /sandbox/gameday/post-recovery ok >/dev/null
      rc=$?
      set -e
      if [[ "$rc" -ne 0 ]]; then
        etcd_status="failed"
...
set +e
etcdctl_cluster put /sandbox/nodes/zone-a-node-1 "$zone_a_payload" >/dev/null
rc1=$?
etcdctl_cluster put /sandbox/nodes/zone-b-node-1 "$zone_b1_payload" >/dev/null
rc2=$?
etcdctl_cluster put /sandbox/nodes/zone-b-node-2 "$zone_b2_payload" >/dev/null
rc3=$?
set -e
if [[ "$rc1" -ne 0 || "$rc2" -ne 0 || "$rc3" -ne 0 ]]; then
  ...
  etcdctl_cluster del /sandbox/nodes/zone-a- --prefix >/dev/null
  set +e
  zone_a_keys="$(etcdctl_cluster get /sandbox/nodes/zone-a- --prefix --keys-only 2>/dev/null)"
  zone_b_keys="$(etcdctl_cluster get /sandbox/nodes/zone-b- --prefix --keys-only 2>/dev/null)"
  set -e
  if grep -q "/sandbox/nodes/zone-a-" <<<"$zone_a_keys"; then
    zone_status="failed"
    zone_failure="zone-a keys still present after simulated zone failure"
  elif ! grep -q "/sandbox/nodes/zone-b-" <<<"$zone_b_keys"; then
    zone_status="failed"
