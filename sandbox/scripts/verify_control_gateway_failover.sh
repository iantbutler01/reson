#!/usr/bin/env bash
# @dive-file: Verifier gate script for verify_control_gateway_failover contract coverage.
...
rg -n "control_gateway_endpoints" "$REPO_ROOT/crates/sandbox/src/lib.rs" >/dev/null
rg -n "candidate_endpoints\(" "$REPO_ROOT/crates/sandbox/src/lib.rs" >/dev/null
rg -n "endpoint_for_new_session\(" "$REPO_ROOT/crates/sandbox/src/lib.rs" >/dev/null
...
report_file_base="$(mktemp "/tmp/chevalier_sandbox_gateway_failover_gate.XXXXXX")"
