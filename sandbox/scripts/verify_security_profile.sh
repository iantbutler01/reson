...
rg -n "request_with_auth|compile_auth_header|ClientTlsConfig|build_client_tls_config" "$REPO_ROOT/crates/sandbox/src/lib.rs" >/dev/null
...
(cd "$REPO_ROOT" && cargo check -p vmd -p chevalier-sandbox --locked)

log "security gate: unit tests"
(cd "$REPO_ROOT" && cargo test -p vmd app::tests::authorize_metadata -- --nocapture)
(cd "$REPO_ROOT" && cargo test -p chevalier-sandbox compile_auth_header -- --nocapture)
