...
rg -n "admission_frozen" "$REPO_ROOT/crates/sandbox/src/distributed.rs" "$REPO_ROOT/vmd/src/config.rs" "$REPO_ROOT/vmd/src/registry.rs" >/dev/null
rg -n "ownership_fence|expected_fence" "$REPO_ROOT/crates/sandbox/src/lib.rs" "$REPO_ROOT/vmd/src/control_bus.rs" >/dev/null
rg -n "continuity_rebinds_session_after_primary_vmd_loss" "$REPO_ROOT/crates/sandbox/tests/facade_contract.rs" >/dev/null

log "planned-drain gate: frozen-node admission exclusion"
(cd "$REPO_ROOT" && cargo test -p chevalier-sandbox --features distributed-control distributed::tests::eligible_routes_skips_admission_frozen_nodes -- --nocapture)

log "planned-drain gate: handoff continuity attach/rebind"
(cd "$REPO_ROOT" && cargo test -p chevalier-sandbox --test facade_contract continuity_rebinds_session_after_primary_vmd_loss -- --nocapture)
