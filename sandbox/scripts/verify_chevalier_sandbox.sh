#!/usr/bin/env bash
# @dive-file: Top-level strict verifier orchestrator for chevalier-sandbox runtime, facade, and distributed HA contract gates.
# @dive-rel: Enforces executable checklist closure from specs/CHEVALIER_SANDBOX_HA_DISTRIBUTED_CONTRACT.md.
...
usage() {
  cat <<'USAGE'
Usage: verify_chevalier_sandbox.sh [--strict] [--with-e2e] [--with-leak-check]
USAGE
}
...
for required_path in \
  ...
  "specs/CHEVALIER_SANDBOX_MIGRATION_CHECKLIST.md" \
  ...
  "specs/CHEVALIER_SANDBOX_ENVELOPE_COMPATIBILITY_POLICY.md" \
  "specs/schemas/control_command_envelope.v1.json" \
  "specs/schemas/control_event_envelope.v1.json" \
  "specs/CHEVALIER_SANDBOX_SLO_THRESHOLDS.json" \
  "crates/sandbox/Cargo.toml"; do
