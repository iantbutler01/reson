#!/usr/bin/env bash
# @dive-file: Verifier gate script for verify_envelope_compat contract coverage.
...
POLICY_DOC="$REPO_ROOT/specs/CHEVALIER_SANDBOX_ENVELOPE_COMPATIBILITY_POLICY.md"
...
rg -n "CONTROL_ENVELOPE_SCHEMA_VERSION|build_command_envelope|build_event_envelope" "$REPO_ROOT/crates/sandbox/src/distributed.rs" >/dev/null

log "envelope-compat gate: envelope unit tests"
(cd "$REPO_ROOT" && cargo test -p chevalier-sandbox command_envelope_contains_required_fields_and_version -- --nocapture)
(cd "$REPO_ROOT" && cargo test -p chevalier-sandbox event_envelope_contains_required_fields_and_version -- --nocapture)
