...
# @dive-rel: Wired into scripts/verify_chevalier_sandbox.sh as checklist gate coverage for item 12.33.
...
rg -n "rebind_session_endpoint\(|is_rebind_candidate_error\(" "$REPO_ROOT/crates/sandbox/src/lib.rs" >/dev/null
rg -n "inflight_exec_rebinds_on_rpc_loss_and_runs_exactly_once|command_invocations" "$REPO_ROOT/crates/sandbox/tests/facade_contract.rs" >/dev/null

log "tier-b exactly-once gate: failover dispatch contract test"
(cd "$REPO_ROOT" && cargo test -p chevalier-sandbox --test facade_contract inflight_exec_rebinds_on_rpc_loss_and_runs_exactly_once -- --nocapture)
