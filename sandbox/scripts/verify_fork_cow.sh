#!/usr/bin/env bash
# @dive-file: Verifier gate script for verify_fork_cow contract coverage.
...
if [[ "${CHEVALIER_SANDBOX_SKIP_RUNNING_FORK_RUNTIME:-0}" == "1" ]]; then
  ...
  warn "running-parent fork runtime sub-check skipped (CHEVALIER_SANDBOX_SKIP_RUNNING_FORK_RUNTIME=1); real gate 42 provides coverage"
...
VM_NAME="chevalier-fork-parent-$(date +%s)"
...
FORK_OUT="$($VMDCTL_BIN --server "$SERVER_URL" --timeout-secs "$TIMEOUT_SECS" fork-vm "$PARENT_VM_ID" --child-name "chevalier-fork-child" --json 2>"$TMP_DIR/fork.stderr")" || {
  err "fork-vm failed"
