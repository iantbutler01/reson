...
rg -n "FenceConflict|current_session_fence|ownership_fence|bind_session_route\\(.*expected_fence" "$REPO_ROOT/crates/sandbox/src/lib.rs" >/dev/null
rg -n "put_session_route\\(.*expected_fence|delete_session_route\\(.*expected_fence|session_fence_key|ownership_fence_allows_transition|Compare::value|Compare::version|Txn::new" "$REPO_ROOT/crates/sandbox/src/distributed.rs" >/dev/null
...
(cd "$REPO_ROOT" && cargo test -p chevalier-sandbox ownership_fence_transition_rejects_stale_expectation -- --nocapture)
