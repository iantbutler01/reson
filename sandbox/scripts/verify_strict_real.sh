...
# @dive-rel: Intended CI/release command for CHEVALIER_SANDBOX_REAL_INTEGRATION_TEST_PLAN.md section 7.7 completion.
set -euo pipefail
...
if [[ "$SKIP_MOCK_PREFLIGHT" -eq 0 ]]; then
  if [[ "$MOCK_PREFLIGHT_STRICT" -eq 1 ]]; then
    ...
    CHEVALIER_SANDBOX_SKIP_RUNNING_FORK_RUNTIME=1 \
      "$REPO_ROOT/scripts/verify_chevalier_sandbox.sh" --strict
  else
    log "strict-real: running fast mock preflight"
    CHEVALIER_SANDBOX_SKIP_RUNNING_FORK_RUNTIME=1 \
      "$REPO_ROOT/scripts/verify_chevalier_sandbox.sh"
  fi
else
