...
if [[ -z "$OUTPUT" ]]; then
  OUTPUT="/tmp/chevalier_sandbox_control_gateway_failover_drill_${timestamp}.txt"
fi
...
set +e
(cd "$REPO_ROOT" && cargo test -p chevalier-sandbox control_gateway_failover_prefers_healthy_secondary_endpoint -- --nocapture)
rc=$?
set -e
if [[ "$rc" -ne 0 ]]; then
  status="failed"
