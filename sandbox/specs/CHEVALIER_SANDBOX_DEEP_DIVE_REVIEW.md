# Chevalier Sandbox Deep Dive Review

Date: 2026-04-24
Scope: `chevalier-sandbox` plus the `OtherYou` API and production manifests that integrate with it.
Status: Review artifact with follow-up fix status appended.

Paths are relative to `/Users/crow/SoftwareProjects/chevalier` unless prefixed with `../OtherYou`.
...
- **Evidence**: Prod sets `CHEVALIER_SANDBOX_PORT_FORWARD_BIND_ADDRESS=0.0.0.0` in `../OtherYou/ops/gke/nym-prod/vmd-statefulset.yaml:137`. QEMU host forwarding uses that bind address in `sandbox/vmd/src/state/manager.rs:2332`. The guest `portproxy` gRPC server binds its address without auth in `sandbox/portproxy/src/main.rs:90`, and exposes shell/file services in `sandbox/portproxy/src/services.rs`.
...
- **Evidence**: Allow/block decisions are authority-regex based in `sandbox/vmd/src/network/mod.rs:611` and `sandbox/vmd/src/network/mod.rs:765`. Envoy dynamic forward proxy resolves hosts via configured DNS in `sandbox/vmd/src/network/mod.rs:553`. Private IP regex blocks are applied to hostname authority strings in `sandbox/vmd/src/network/mod.rs:827`, not to resolved upstream IPs.
...
- **Impact**: Security Critical, Correctness Medium, Speed Low.
- **Evidence**: Firewall rules allow private-range UDP high ports before dropping private CIDRs in `sandbox/vmd/src/network/firewall.rs:159`, allow WebRTC UDP ports in `sandbox/vmd/src/network/firewall.rs:236`, and finally allow UDP ephemeral range in `sandbox/vmd/src/network/firewall.rs:273`.
...
- **Evidence**: vmd TLS only enables when TLS config exists in `sandbox/vmd/src/app.rs:143` and `sandbox/vmd/src/config.rs:1060`. Prod starts and advertises `http://...:8052` in `../OtherYou/ops/gke/nym-prod/vmd-statefulset.yaml:121` and `../OtherYou/ops/gke/nym-prod/vmd-statefulset.yaml:135`. The API connects over HTTP in `../OtherYou/ops/gke/nym-api/configmap.yaml:15` while attaching bearer metadata in `../OtherYou/api/src/services/computer/chevalier.rs:170`. Internal NymFS auth also uses bearer headers in `sandbox/vmd/src/fuse/client.rs:56` and `../OtherYou/api/src/middleware/internal_auth.rs:23`.
...
- **Evidence**: Prod config uses `http://` etcd and `nats://` NATS in `../OtherYou/ops/gke/nym-prod/vmd-configmap.yaml:12` and `../OtherYou/ops/gke/nym-prod/vmd-configmap.yaml:13`. etcd listens on HTTP in `../OtherYou/ops/gke/nym-prod/etcd.yaml:83`. NATS now requires a token from `chevalier-vmd-auth/internal-service-token`, but transport is still plaintext and etcd is still unauthenticated/plaintext.
...
- **Impact**: Security High, Correctness Medium, Speed Low.
- **Evidence**: vmd public ingress authorizes using bearer/cookie headers in `sandbox/vmd/src/public_ingress.rs:156`, then copies all request headers except connection/transfer/content-length to the guest in `sandbox/vmd/src/public_ingress.rs:304`. The safer API ingress path strips `Authorization` and sanitizes cookies in `../OtherYou/api/src/routes/vm_ingress.rs:73`.
...
- **Impact**: Security High, Correctness Medium, Speed Low.
- **Evidence**: VM startup maps the public proxy port to guest proxy port `13337` in `sandbox/vmd/src/state/manager.rs:1307`. Public ingress writes the requested guest port preface to that proxy in `sandbox/vmd/src/public_ingress.rs:273`. API-generated ingress records carry arbitrary `guest_port` values into VM metadata in `../OtherYou/api/src/services/computer/chevalier.rs:1375`.
...
- **Impact**: Security High, Correctness Medium, Speed Low.
- **Evidence**: VM launch recursively `chown`s before setuid in `sandbox/vmd/src/state/manager.rs:2830`. Traversal calls `chown_single_path` on directories and entries in `sandbox/vmd/src/state/manager.rs:2886` and `sandbox/vmd/src/state/manager.rs:2902`; `chown_single_path` uses `libc::chown` in `sandbox/vmd/src/state/manager.rs:2919`.
...
- **Impact**: Security High, Correctness High, Speed Low.
- **Evidence**: cgroup path extraction is best-effort in `sandbox/vmd/src/network/firewall.rs:590`. Block rules use cgroup path matching in `sandbox/vmd/src/network/firewall.rs:377`. Manager logs and continues on registration failure in `sandbox/vmd/src/state/manager.rs:1540`, and the enforcer skips missing pid data in `sandbox/vmd/src/network/mod.rs:899`.
...
- **Evidence**: Default prebuilt image registry/reference is configured in `sandbox/vmd/src/image.rs:24` and `sandbox/vmd/src/image.rs:51`. Downloads stream raw bytes into a target file in `sandbox/vmd/src/image.rs:207` and publish with rename in `sandbox/vmd/src/image.rs:251`. Cache trust is based on local existence/size in `sandbox/vmd/src/state/manager.rs:1802`.
...
- **Impact**: Security High, Correctness Medium, Speed Low.
- **Evidence**: Docker fallback runs d2vm with `--privileged` and `/var/run/docker.sock` mounted in `sandbox/vmd/src/virt/mod.rs:140`. Prod sets `CHEVALIER_SANDBOX_D2VM_IMAGE` to `:latest` in `../OtherYou/ops/gke/nym-prod/vmd-configmap.yaml:7`; the vmd app image is tag-pinned but not digest-pinned in `../OtherYou/ops/gke/nym-prod/vmd-statefulset.yaml:93`.
...
- **Evidence**: The facade sends timeout/detach fields in `sandbox/crates/sandbox/src/lib.rs:1013` and `sandbox/crates/sandbox/src/lib.rs:1038`. vmd explicitly ignores them in `sandbox/vmd/src/control_bus.rs:1055`.
...
- **Evidence**: Subscriber decodes then ACKs before event channel delivery is proven in `sandbox/crates/sandbox/src/distributed.rs:1020` and `sandbox/crates/sandbox/src/distributed.rs:1021`; the bounded channel can fail in `sandbox/crates/sandbox/src/distributed.rs:1068`. Event decode fills missing stream ids rather than validating against the subscribed stream in `sandbox/crates/sandbox/src/distributed.rs:1736`.
...
- **Impact**: Security Medium, Correctness High, Speed Low.
- **Evidence**: Dedupe can fall back to local-only or continue without short-circuit in `sandbox/vmd/src/control_bus.rs:722`. Stdin input carries sequence metadata but does not enforce monotonic application in `sandbox/vmd/src/control_bus.rs:1774`.
...
- **Impact**: Security Medium, Correctness High, Speed Low.
- **Evidence**: Facade session-route writes use route `expected_fence` in `sandbox/crates/sandbox/src/lib.rs:2115` and `sandbox/crates/sandbox/src/lib.rs:2128`. vmd control-bus commands check a separate `/command-dedupe/ownership-fences` keyspace in `sandbox/vmd/src/control_bus.rs:231`.
...
- **Impact**: Security Medium, Correctness High, Speed Medium.
- **Evidence**: Normal route writes use sharded route/fence records and delete legacy keys in `sandbox/crates/sandbox/src/distributed.rs:316` and `sandbox/crates/sandbox/src/distributed.rs:357`. Reconciliation writes legacy `/sessions/{session_id}` values in `sandbox/vmd/src/reconcile.rs:216`, while route decoding defaults missing fields in `sandbox/crates/sandbox/src/distributed.rs:1421`.
...
- **Evidence**: Distributed node selection counts current routes before create in `sandbox/crates/sandbox/src/distributed.rs:506`; the session route is bound after VM creation in `sandbox/crates/sandbox/src/lib.rs:2248` and `sandbox/crates/sandbox/src/lib.rs:2267`. Local manager checks capacity before insert in `sandbox/vmd/src/state/manager.rs:394` and inserts later in `sandbox/vmd/src/state/manager.rs:529`.
...
- **Impact**: Security Low, Correctness High, Speed Low.
- **Evidence**: Restore reverts disk before checking the RAM file exists in `sandbox/vmd/src/state/manager.rs:1727` and `sandbox/vmd/src/state/manager.rs:1731`. Fork pauses the parent before snapshot and can error before resume/cleanup in `sandbox/vmd/src/state/manager.rs:623` and `sandbox/vmd/src/state/manager.rs:636`.
...
- **Impact**: Security Low, Correctness High, Speed Medium.
- **Evidence**: FUSE lazily loads whole-file handles, writes into an in-memory buffer, and flushes the whole buffer through `sandbox/vmd/src/fuse/fs.rs`. The client PUT has no server-side previous-hash/version precondition in `sandbox/vmd/src/fuse/client.rs`.
...
- **Impact**: Security Low, Correctness High, Speed Low.
- **Evidence**: `open` ignores `_flags` in `sandbox/vmd/src/fuse/fs.rs:555`, lazy loading reads existing content in `sandbox/vmd/src/fuse/fs.rs:363`, writes overlay bytes in `sandbox/vmd/src/fuse/fs.rs:625`, and flush writes the resulting whole buffer in `sandbox/vmd/src/fuse/fs.rs:309`.
...
- **Impact**: Security Low, Correctness High, Speed Medium.
- **Evidence**: `stop_vm` returns metadata immediately for `VmState::Paused` in `sandbox/vmd/src/state/manager.rs:1552`; `delete_vm` rejects paused VMs in `sandbox/vmd/src/state/manager.rs:582`. OtherYou's stop helper no-ops unless state is running in `../OtherYou/api/src/services/computer/chevalier.rs:1008`.
...
- **Impact**: Security Medium, Correctness High, Speed Medium.
- **Evidence**: NATS is `replicas: 1` in `../OtherYou/ops/gke/nym-prod/nats.yaml:52`; etcd is `replicas: 1` in `../OtherYou/ops/gke/nym-prod/etcd.yaml:46`; JetStream stream replicas default to 1 in `sandbox/crates/sandbox/src/distributed.rs:1355`.
...
- **Impact**: Security Low, Correctness High, Speed Low.
- **Evidence**: HA mode now requires node registry/control bus configuration, starts the node registry before the partition monitor, and treats an etcd probe with no visible node-registry key as a partition failure. Defaults remain a 2 second probe and threshold 3 in `sandbox/vmd/src/partition.rs:14`.
...
- **Evidence**: Facade resource options are optional in `sandbox/crates/sandbox/src/lib.rs:250`; vmd clamps only to non-negative values in `sandbox/vmd/src/app.rs:503`; manager applies minimum defaults in `sandbox/vmd/src/state/manager.rs:385`; QEMU uses the resulting values in `sandbox/vmd/src/state/manager.rs:2347`.
...
- **Evidence**: API launch URLs issue `?nym_ingress_token=...` in `../OtherYou/api/src/services/nym_vm_ingress_service.rs:161`. The API middleware path accepts that token in `../OtherYou/api/src/routes/vm_ingress.rs:40`. The vmd public ingress path only accepts bearer/cookie and forwards those to internal auth in `sandbox/vmd/src/public_ingress.rs:156` and `sandbox/vmd/src/public_ingress.rs:220`. The generated Kubernetes ingress points at the vmd service/port in `../OtherYou/api/src/services/nym_vm_ingress_service.rs:412`.
...
- **Impact**: Security Medium, Correctness Medium, Speed High.
- **Evidence**: Route resolution loops through `manager.list()` for every request in `sandbox/vmd/src/public_ingress.rs:174` and `sandbox/vmd/src/state/manager.rs:313`. Request bodies use `to_bytes(body, usize::MAX)` in `sandbox/vmd/src/public_ingress.rs:269`, and responses are collected fully in `sandbox/vmd/src/public_ingress.rs:327`.
...
- **Impact**: Security Medium, Correctness Medium, Speed High.
- **Evidence**: Control-bus run exec appends stdout/stderr into strings in `sandbox/vmd/src/control_bus.rs:918` through `sandbox/vmd/src/control_bus.rs:938`, then publishes one result payload.
...
- **Impact**: Security Medium, Correctness Medium, Speed High.
- **Evidence**: `ReadFile` returns one `bytes data` payload in `sandbox/proto/bracket/portproxy/v1/portproxy.proto:99`. Service `read_file` uses `fs::read` in `sandbox/portproxy/src/services.rs:466`, `write_file` accepts the full request body in `sandbox/portproxy/src/services.rs:483`, and `list_directory` accumulates entries in `sandbox/portproxy/src/services.rs:508`.
...
- **Evidence**: Write resizes the in-memory buffer in `sandbox/vmd/src/fuse/fs.rs:625`; flush writes the whole buffer to the remote service in `sandbox/vmd/src/fuse/fs.rs:309`; loading a handle can fetch the whole file in `sandbox/vmd/src/fuse/fs.rs:371`.
...
- **Impact**: Security Low, Correctness High, Speed Medium.
- **Evidence**: Internal `put_file` takes `Bytes` in `../OtherYou/api/src/routes/runtime_nymfs_internal.rs:279`, and no body-limit override was found under `../OtherYou/api/src`. Local `axum-core` default limit is `2_097_152` in `/Users/crow/.cargo/registry/src/index.crates.io-1949cf8c6b5b557f/axum-core-0.5.6/src/ext_traits/request.rs:319`. FUSE flush sends one full PUT body in `sandbox/vmd/src/fuse/fs.rs:309`.
...
- **Evidence**: FUSE `readdir` calls `list_dir` in `sandbox/vmd/src/fuse/fs.rs:216`. Internal `get_tree` hashes each file in `../OtherYou/api/src/routes/runtime_nymfs_internal.rs:121`; GCS async hashing fetches object bytes in `../OtherYou/api/src/services/runtime/gcs_nym_fs_adapter.rs:1899`.
...
- **Impact**: Security Low, Correctness Medium, Speed High.
- **Evidence**: FUSE uses ranged reads for large files in `sandbox/vmd/src/fuse/fs.rs:235`. GCS `read_range_async` fetches the full packed slot and slices locally in `../OtherYou/api/src/services/runtime/gcs_nym_fs_adapter.rs:2097` through `../OtherYou/api/src/services/runtime/gcs_nym_fs_adapter.rs:2126`.
...
- **Impact**: Security Low, Correctness Medium, Speed High.
- **Evidence**: Node selection calls `session_route_records` in `sandbox/crates/sandbox/src/distributed.rs:506`; that scans `/sessions/` with prefix reads in `sandbox/crates/sandbox/src/distributed.rs:1188`; counts are recomputed in `sandbox/crates/sandbox/src/distributed.rs:1572`.
...
- **Impact**: Security Low, Correctness Medium, Speed High.
- **Evidence**: Node registry creates/connects and grants a lease each heartbeat in `sandbox/vmd/src/registry.rs:99`. Port lease heartbeat similarly creates clients/leases in `sandbox/crates/sandbox/src/distributed.rs:1095` and `sandbox/crates/sandbox/src/distributed.rs:1856`.
...
- **Impact**: Security Low, Correctness Medium, Speed High.
- **Evidence**: etcd dedupe store writes command keys in `sandbox/vmd/src/control_bus.rs:220` without an attached lease. Ownership fence keys live under `/ownership-fences` in `sandbox/vmd/src/control_bus.rs:242`. Reconcile summaries are written under `/reconcile/` in `sandbox/vmd/src/reconcile.rs:245`.
...
- **Impact**: Security Low, Correctness Medium, Speed High.
- **Evidence**: Directory cache inserts without a capacity cap in `sandbox/vmd/src/fuse/cache.rs:96`; inode table ensures new entries without eviction in `sandbox/vmd/src/fuse/fs.rs:85` and `sandbox/vmd/src/fuse/fs.rs:135`.
...
- **Impact**: Security Medium, Correctness Medium, Speed High.
- **Evidence**: Download temp path is deterministic `{}.part` in `sandbox/vmd/src/image.rs:112`; it resumes and renames without a per-image process lock in `sandbox/vmd/src/image.rs:129` and `sandbox/vmd/src/image.rs:251`. Create/pre-download paths can both invoke download in `sandbox/vmd/src/state/manager.rs:1844` and `sandbox/vmd/src/app.rs:835`.
...
- **Impact**: Security Low, Correctness Medium, Speed High.
- **Evidence**: Envoy writes access logs in `sandbox/vmd/src/network/mod.rs:700`. The enforcer polls periodically in `sandbox/vmd/src/network/mod.rs:876`. Counters read new log data into a string in `sandbox/vmd/src/network/vm_counters.rs:165` and store counters in an unbounded `by_vm` map in `sandbox/vmd/src/network/vm_counters.rs:62`.
...
- **Evidence**: Direct exec starts a child in `sandbox/portproxy/src/services.rs:105`; timeout calls `child.start_kill()` only on the top-level process in `sandbox/portproxy/src/services.rs:207`; daemon commands have no timeout in `sandbox/portproxy/src/daemon.rs:66`; child tracker stores top-level PIDs in `sandbox/portproxy/src/child_tracker.rs:22`.
...
- **Evidence**: Local `cargo check --workspace` in `chevalier-sandbox` fails with stable rustc 1.86.0 because `if let` chains are used in `sandbox/crates/sandbox/src/lib.rs:2397`; no `rust-toolchain` was present to pin nightly/stable expectations.
...
- **Evidence**: Native d2vm args include `--password root` in `sandbox/vmd/src/virt/mod.rs:116`; Docker d2vm args do the same in `sandbox/vmd/src/virt/mod.rs:165`.
...
- Static validation passed for `chevalier-sandbox`: `cargo fmt --all -- --check`, `cargo check --workspace`, `cargo test -p vmd --lib`, `cargo test -p chevalier-sandbox --lib`, `cargo test -p portproxy`, and `git diff --check`.
