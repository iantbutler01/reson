...
class TypeParser(OutputParser[T]):
    ...
    def parse(self, output: str, output_type: Type[T]) -> ParserResult[T]:
        ...
        try:
            ...
            return ParserResult(value=value, is_partial=False, raw_output=output)
        except Exception as e:
            print("WE HIT AN ERROR AND DIED: {e}")
            return ParserResult(error=e, raw_output=output)
    ...
    def validate_final(self, parser) -> ParserResult[T]:
        ...
        try:
            ...
            return ParserResult(
                value=value, is_partial=False, raw_output=getattr(parser, "_buffer", "")
            )
        except Exception as e:
            print(f"Validation error: {e}")
            return ParserResult(
                error=e, is_partial=False, raw_output=getattr(parser, "_buffer", "")
            )
    ...
    def enhance_prompt(
        ...
        if output_type is not None:
            try:
                ...
                processed_prompt = interpolate_prompt(
                    processed_prompt.replace("{return_type}", "{{return_type}}"),
                    output_type,
                    format_tag="return_type",
                )
            except Exception as e:
                print(
                    f"Error enhancing prompt with type information (interpolate_prompt): {e}"
                )
        ...
        if call_context:
            try:
                ...
                if not hasattr(self, "_jinja_env") or self._jinja_env is None:
                    ...
                    self._jinja_env.filters["json"] = lambda obj: json.dumps(
                        obj, indent=2
                    )
    ...
    def _create_parser(self, output_type: Type[T]) -> Parser:
        ...
        try:
            if (
                hasattr(output_type, "__mro__")
                and Deserializable in output_type.__mro__
            ):
                return Parser(output_type)
        except TypeError:
    ...
    def _convert_to_typed_result(self, result: Any, output_type: Type[T]) -> Any:
                ...
                try:
                    ...
                    for key, value in result.items():
                        ...
                        if field_type is not None:
                            ...
                            converted_dict[key] = self._convert_to_typed_result(
                                value, field_type
                            )
                        else:
                            converted_dict[key] = value
        ...
        if origin_type == list and isinstance(result, list) and type_args:
            ...
            try:
                return [
                    self._convert_to_typed_result(item, item_type) for item in result
                ]
            except Exception as e:
        ...
        if origin_type == dict and isinstance(result, dict) and len(type_args) == 2:
            ...
            try:
                return {
                    k: self._convert_to_typed_result(v, value_type)
