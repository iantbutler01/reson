...
async fn test_tracing_client_anthropic_cost_tracking() {
    ...
    assert!(
        credits < 1_000_000,
        "Cost seems too high: {} microdollars",
        credits
    );
}
...
async fn test_tracing_client_callback_invoked() {
    ...
    let callback: reson_agentic::providers::TraceCallback =
...
async fn test_cost_info_calculations() {
    ...
    println!(
        "GPT-4o-mini cost: {} microdollars",
        cost_mini.microdollar_cost
    );
    ...
    println!(
        "Sonnet with cache cost: {} microdollars",
        cost_cached.microdollar_cost
    );

    // Test dollar conversion
    println!(
        "10500 microdollars = ${}",
        CostInfo::from_usage(&usage, "claude-3-sonnet").total_dollars()
    );

    // Test provider-reported cost (e.g., from OpenRouter)
    let provider_cost_dollars = 0.0025; // $0.0025 = 2500 microdollars
    let cost_with_provider =
        CostInfo::from_usage_with_provider_cost(&usage, "unknown-model", provider_cost_dollars);
    ...
    println!(
        "Provider cost: {} microdollars (${:.6})",
        cost_with_provider.total_microdollars(),
        cost_with_provider.total_dollars()
    );

    // Test with_provider_cost builder
    let cost_builder = CostInfo::from_usage(&usage, "claude-3-sonnet").with_provider_cost(0.005); // $0.005 = 5000 microdollars
