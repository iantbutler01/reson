...
//! `RESON_LIVE_IMAGE_TESTS=1` is present.
...
use reson_agentic::types::{MediaPart, MediaSource, MultimodalMessage};
...
fn live_image_tests_enabled() -> bool {
    std::env::var("RESON_LIVE_IMAGE_TESTS")
...
#[tokio::test]
#[ignore = "Requires RESON_LIVE_IMAGE_TESTS=1 and OPENAI_API_KEY"]
async fn live_openai_chat_image_input_non_streaming_and_streaming() {
    if !live_image_tests_enabled() {
        eprintln!("Skipping: RESON_LIVE_IMAGE_TESTS not enabled");
        return;
    }

    let api_key = std::env::var("OPENAI_API_KEY").expect("OPENAI_API_KEY not set");
    let model = std::env::var("RESON_LIVE_OPENAI_IMAGE_MODEL")
...
async fn live_openai_responses_image_input_non_streaming_and_streaming() {
    ...
    let api_key = std::env::var("OPENAI_API_KEY").expect("OPENAI_API_KEY not set");
    let model = std::env::var("RESON_LIVE_OPENAI_RESPONSES_IMAGE_MODEL")
...
#[tokio::test]
#[ignore = "Requires RESON_LIVE_IMAGE_TESTS=1 and ANTHROPIC_API_KEY"]
async fn live_anthropic_image_input_non_streaming_and_streaming() {
    ...
    let model = std::env::var("RESON_LIVE_ANTHROPIC_IMAGE_MODEL")
...
#[tokio::test]
#[ignore = "Requires RESON_LIVE_IMAGE_TESTS=1 and GOOGLE_GEMINI_API_KEY"]
async fn live_google_gemini_image_input_non_streaming_and_streaming() {
    ...
    let model = std::env::var("RESON_LIVE_GOOGLE_IMAGE_MODEL")
        .unwrap_or_else(|_| "gemini-2.5-flash".to_string());
    let client = reson_agentic::providers::GoogleGenAIClient::new(api_key, &model);
    let messages = letter_l_message();
    let config = GenerationConfig::new(&model).with_max_tokens(64);
...
#[tokio::test]
#[ignore = "Requires RESON_LIVE_IMAGE_TESTS=1 and OPENROUTER_API_KEY"]
async fn live_openrouter_chat_image_input_non_streaming_and_streaming() {
    ...
    let model = std::env::var("RESON_LIVE_OPENROUTER_IMAGE_MODEL")
...
async fn live_openrouter_responses_image_input_non_streaming_and_streaming() {
    ...
    let api_key = std::env::var("OPENROUTER_API_KEY").expect("OPENROUTER_API_KEY not set");
    let model = std::env::var("RESON_LIVE_OPENROUTER_RESPONSES_IMAGE_MODEL")
...
#[tokio::test]
#[ignore = "Requires RESON_LIVE_IMAGE_TESTS=1 and GOOGLE_APPLICATION_CREDENTIALS"]
async fn live_google_anthropic_image_input_non_streaming_and_streaming() {
    ...
    let model = std::env::var("RESON_LIVE_GOOGLE_ANTHROPIC_IMAGE_MODEL")
        .unwrap_or_else(|_| "claude-3-5-sonnet-v2@20241022".to_string());
    let region = std::env::var("RESON_LIVE_GOOGLE_ANTHROPIC_REGION")
        .unwrap_or_else(|_| "us-east5".to_string());
    let client = reson_agentic::providers::GoogleAnthropicClient::from_adc(&model, region);
    let messages = letter_l_message();
    let config = GenerationConfig::new(&model).with_max_tokens(64);
...
#[cfg(feature = "bedrock")]
#[tokio::test]
#[ignore = "Requires RESON_LIVE_IMAGE_TESTS=1 and AWS Bedrock credentials"]
async fn live_bedrock_claude_image_input_non_streaming_and_streaming() {
    ...
    let model = std::env::var("RESON_LIVE_BEDROCK_IMAGE_MODEL")
        .unwrap_or_else(|_| "anthropic.claude-3-5-haiku-20241022-v1:0".to_string());
    let region = std::env::var("AWS_REGION")
        .or_else(|_| std::env::var("AWS_DEFAULT_REGION"))
        .ok();
    let client = reson_agentic::providers::BedrockClient::new(&model, region);
    let messages = letter_l_message();
    let config = GenerationConfig::new(&model).with_max_tokens(64);
