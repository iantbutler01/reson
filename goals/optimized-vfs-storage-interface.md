...
Build one canonical, optimized VFS storage interface in a Reson VFS namespace/crate that every VFS consumer uses, whether the implementation is local/in-process or remote over the Reson VFS gateway.

This is the next migration stage after the generic VFS gateway/FUSE boundary. The current intermediate step proves the gateway shape works. The next step is to move the durable VFS storage mechanics into Reson without losing the performance and correctness properties already optimized in OtherYou.

Current-state assumption: the optimized durable VFS storage path still lives in OtherYou today. Reson currently owns the generic HTTP gateway, VMD/FUSE client behavior, VM lifecycle, and distributed control mechanics. This goal is not to replace that working storage path with a new design; it is to lift the proven storage primitives into a Reson VFS namespace behind one interface.

Namespace requirement: generic VFS storage must be a sibling Reson namespace, not a child of `reson-sandbox`. `reson-vfs` should own pack/manifest/storage primitives and optimized local/remote storage interfaces. `reson-sandbox` should remain a consumer/integration layer for VM, FUSE, and gateway behavior.

## Core Direction

`reson-vfs` should own generic VFS storage mechanics:
...
- batched lookup methods matching the `reson-vfs` manifest/index trait
...
- examples/config that let a new Reson user start with local Postgres and object/file storage without writing their own index layer
...
- no product-owned `read_many`/`read_range`/`write_many_atomic` storage implementation except as a thin policy call into `reson-vfs`
- no product-owned metadata batching or changed-only write planner except where it is genuinely product policy

OtherYou may keep a small product bridge that supplies account/nym identity, logical mount roots, authorization/lease decisions, mutation ledger recording, and product database/repository access to the generic `reson-vfs` implementation. That bridge should not reimplement storage mechanics.

## Interface Requirement

Expose one optimized storage trait/API in `reson-vfs` for:
...
This is a target storage interface, not merely the current Reson `VfsGatewayBackend` trait. The current gateway is per-file HTTP/FUSE plumbing in `reson-sandbox`; the target interface lives beside that layer and must also preserve the bulk/batch primitives that currently make OtherYou's storage path fast.
...
- remote/gateway mode: client adapter speaking to the Reson VFS HTTP gateway exposed by sandbox/product integration
...
1. Extract the pack/manifest/zstd primitives from OtherYou into `reson-vfs` without changing behavior.
2. Extract the Postgres-backed manifest/index implementation from OtherYou into `reson-vfs`, preserving the production schema semantics, transaction behavior, batching, and query performance.
3. Add a local/direct `reson-vfs` implementation and prove it against the old OtherYou test corpus.
4. Add a remote/gateway `reson-vfs` implementation that uses the same interface.
5. Adapt OtherYou to provide product policy hooks and consume the lifted Reson implementation instead of owning generic storage mechanics.
6. Replace the old OtherYou adapter files/call paths with `reson-vfs` consumers once parity is proven.
...
- The active metadata-backed GCS path calls into `reson-vfs::ObjectBackedVfsStorage` for the canonical storage operations. The remaining OtherYou adapter code is a product bridge for path projection, protected-write policy, mutation recording, digest refresh, TTL cache bridging, and compatibility fallback behavior.
- The one remaining direct pack-slot extraction in OtherYou is in the `read_range_async` compatibility fallback after the primary `reson-vfs` range-read path misses. It is not the canonical metadata-backed path.
...
- Internal Reson VFS raw read returned `vfs-ok`.
- Internal Reson VFS ranged read `bytes=0-2` returned `vfs`.
...
- A no-LLM internal Reson VFS validation against the same Nym/conversation verified the protected shared mount path:
...
- Local validation artifacts under `conversations/c136f087-03eb-41a1-acf7-521f7d84d074/shared/vfs-*` were deleted through the internal Reson VFS API after the check.
- A real LLM-backed developer-mode VM/FUSE validation then exercised the product path through model tool selection, not only direct API calls:
  ...
  - internal Reson VFS raw read returned `llm-vm-vfs-ok`
...
- A second real LLM-backed validation exercised VM pause/resume and warm reattach around the same mounted shared file:
  ...
  - internal Reson gateway raw read returned `llm-vm-resume-ok`; legacy `/internal/nymfs` raw read also returned `llm-vm-resume-ok`
...
- The two LLM-created shared validation files were then cleaned up through the protected Reson VFS gateway delete path:
...
- Focused automated validation after the fix:
  ...
  - `cargo test --manifest-path reson-vfs/Cargo.toml` passed: 39 passed
  - `cargo test --manifest-path reson-sandbox/crates/reson-sandbox/Cargo.toml vfs -- --nocapture` passed: 4 passed, remaining filtered out
  - `git diff --check` passed in both OtherYou and Reson
- Feature-gated Reson VFS validation after the completion audit:
  - `rustup run 1.90-aarch64-apple-darwin cargo test --manifest-path reson-vfs/Cargo.toml --features postgres` passed: 42 passed
  - `rustup run 1.90-aarch64-apple-darwin cargo test --manifest-path reson-vfs/Cargo.toml --features gateway` passed: 45 passed
  - `rustup run 1.90-aarch64-apple-darwin cargo test --manifest-path reson-vfs/Cargo.toml --features postgres,gateway,gcs,tracing` passed: 48 passed
  - `rustup run 1.90-aarch64-apple-darwin cargo check --manifest-path reson-vfs/Cargo.toml --example postgres_local_object_store --features postgres,tokio` passed
  - the `postgres_local_object_store` example was live-tested against the local Postgres server after applying `migrations/postgres/001_reson_vfs_index.sql`; it wrote and read `hello from reson-vfs`, then the temporary `reson_vfs_*` tables and local object directory were cleaned up
  - `rustup run 1.90-aarch64-apple-darwin cargo test --manifest-path reson-sandbox/crates/reson-sandbox/Cargo.toml --test facade_contract control_gateway_failover_prefers_healthy_secondary_endpoint -- --nocapture` passed
...
- Adopter-facing Reson VFS docs and a local Postgres/object-store example were added:
  - `reson-vfs/README.md`
  - `reson-vfs/examples/postgres_local_object_store.rs`
  - the README points to `migrations/postgres/001_reson_vfs_index.sql`, explains the object-store + manifest/index split, and shows local Postgres plus local-object-store setup
...
- At the start of this goal, the optimized storage mechanics were in OtherYou, not Reson. OtherYou owned the GCS adapter, manifests, file versions, pack rows, zstd pack building, batch reads, atomic bulk writes, range reads, subtree prefetch, and compaction. Current branch state has lifted the generic pack/cache/object-store/index/gateway/compaction pieces into `reson-vfs`, while OtherYou still owns the product bridge and compatibility cutover paths.
- Current Reson VFS code in `reson-sandbox` is a gateway/backend boundary for VM/FUSE access. It has per-file methods, range reads through `read_file(..., range)`, namespace mutations, and lease calls, but it is not yet the full optimized storage abstraction described here.
...
- DB-backed manifests are an OtherYou implementation choice for the current packed durable path, not a fundamental Reson VFS dependency. `reson-vfs` should require a manifest/index abstraction for packed storage while allowing DB-backed, local sidecar-backed, or gateway-delegated implementations. Reson should nevertheless provide a first-class Postgres-backed index implementation so a new project is not forced to invent one before using packed VFS storage. That Postgres implementation should be based on the production-tested OtherYou implementation and validated by direct parity, not designed as a fresh parallel interpretation.
- Product ownership, logical path mapping, protected write policy, resource-key/lease validation, mutation ledger rows, and product sidecars remain OtherYou policy. They should call into Reson storage rather than being absorbed into generic Reson storage semantics.
- Existing OtherYou adapter files are migration source material, not the desired final architecture. Their storage responsibilities should either move to `reson-vfs` or disappear behind a thin product bridge.
...
Carry forward the operational constraints from `reson/migration-goal.md`:
...
- leave Reson and OtherYou with only intentional code changes and commit the final work

## Definition Of Done

There is exactly one optimized VFS storage abstraction in `reson-vfs`. Local and remote are implementation choices under that abstraction. `reson-sandbox` consumes the abstraction for VM/FUSE/gateway integration. OtherYou consumes `reson-vfs` and no longer has product-owned VFS adapter files as the canonical storage implementation, but still owns product policy and product-specific persistence meaning.

Performance and correctness are proven against the old optimized OtherYou behavior before production deployment.

Reson also includes a supported default manifest/index implementation, with Postgres as the recommended batteries-included backend. That implementation is the lifted, generalized form of OtherYou's production-tested VFS index path. A new Reson adopter can run packed VFS storage with documented local Postgres setup, migrations/schema, and the same optimized read/write/index semantics without depending on OtherYou code, and OtherYou can consume the Reson version without losing the behavior that has already proven itself in production.
