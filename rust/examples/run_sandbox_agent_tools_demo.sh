...
export CHEVALIER_SANDBOX_DATA_DIR="${CHEVALIER_SANDBOX_DATA_DIR:-/tmp/chevalier-agent-tools-vmd}"
...
export CHEVALIER_EXEC_READY_TIMEOUT_SECS="${CHEVALIER_EXEC_READY_TIMEOUT_SECS:-240}"
export CHEVALIER_AGENT_MODEL="${CHEVALIER_AGENT_MODEL:-${LOCAL_LLM:-}}"

if [[ -z "${CHEVALIER_AGENT_MODEL}" ]]; then
  cat >&2 <<'ERR'
missing model config:
  set CHEVALIER_AGENT_MODEL (preferred) or LOCAL_LLM

examples:
  export CHEVALIER_AGENT_MODEL='openrouter:anthropic/claude-sonnet-4'
