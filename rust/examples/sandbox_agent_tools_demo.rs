...
//! - `CHEVALIER_AGENT_MODEL` (or `LOCAL_LLM`)
...
fn exec_ready_timeout_secs() -> u64 {
    std::env::var("CHEVALIER_EXEC_READY_TIMEOUT_SECS")
        .ok()
...
fn resolve_agent_model() -> Result<String, Box<dyn std::error::Error>> {
    if let Ok(model) = std::env::var("CHEVALIER_AGENT_MODEL") {
    ...
    Err("missing model config: set CHEVALIER_AGENT_MODEL or LOCAL_LLM".into())
}
...
fn tool_err(message: impl Into<String>) -> ChevalierError {
    ChevalierError::NonRetryable(message.into())
}
...
async fn discard_with_retry(
    ...
    let discard_timeout_secs = std::env::var("CHEVALIER_DISCARD_TIMEOUT_SECS")
        .ok()
        .and_then(|value| value.parse::<u64>().ok())
        .unwrap_or(30);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mode = std::env::var("CHEVALIER_SANDBOX_MODE").unwrap_or_else(|_| "local".to_string());
    let endpoint = std::env::var("CHEVALIER_SANDBOX_ENDPOINT").ok();
    let image = std::env::var("CHEVALIER_SANDBOX_IMAGE")
        .unwrap_or_else(|_| "ghcr.io/bracketdevelopers/uv-builder:main".to_string());
    let attach_session_id = std::env::var("CHEVALIER_ATTACH_SESSION_ID").ok();
        ...
        == Some("1")
    {
        let etcd_endpoints = std::env::var("CHEVALIER_SANDBOX_ETCD_ENDPOINTS")
            ...
            .collect::<Vec<_>>();
        let subject_prefix = std::env::var("CHEVALIER_SANDBOX_NATS_SUBJECT_PREFIX")
            .unwrap_or_else(|_| "chevalier.sandbox.control".to_string());
        let dist_cfg = DistributedControlConfig {
            etcd_endpoints,
            etcd_prefix: std::env::var("CHEVALIER_SANDBOX_ETCD_PREFIX")
                .unwrap_or_else(|_| "/chevalier-sandbox".to_string()),
            nats_url: std::env::var("CHEVALIER_SANDBOX_NATS_URL")
                .unwrap_or_else(|_| "nats://127.0.0.1:4222".to_string()),
            nats_subject_prefix: subject_prefix.clone(),
            nats_stream_name: std::env::var("CHEVALIER_SANDBOX_NATS_STREAM")
                .unwrap_or_else(|_| "CHEVALIER_SANDBOX_CONTROL".to_string()),
            nats_stream_max_age_secs: std::env::var("CHEVALIER_SANDBOX_NATS_STREAM_MAX_AGE_SECS")
                .ok()
                .and_then(|value| value.parse::<u64>().ok())
                .unwrap_or(60 * 60 * 24 * 7),
            nats_stream_replicas: std::env::var("CHEVALIER_SANDBOX_NATS_STREAM_REPLICAS")
                .ok()
    ...
    let sandbox = if mode == "remote" {
        let endpoint = endpoint.ok_or_else(|| {
            "CHEVALIER_SANDBOX_ENDPOINT is required when CHEVALIER_SANDBOX_MODE=remote".to_string()
        })?;
        println!("mode=remote endpoint={endpoint}");
        Sandbox::connect(endpoint, cfg).await?
    } else {
        let vmd_bin = std::env::var("CHEVALIER_VMD_BIN")
            .unwrap_or_else(|_| "../sandbox/target/debug/vmd".to_string());
        let proxy_bin_dir = std::env::var("CHEVALIER_PROXY_BIN_DIR")
            .unwrap_or_else(|_| "../sandbox/portproxy/bin".to_string());
        let data_dir = std::env::var("CHEVALIER_SANDBOX_DATA_DIR")
            .unwrap_or_else(|_| "/tmp/chevalier-agent-tools-vmd".to_string());
    ...
    let (session, attached_parent, session_id) = if let Some(existing) = attach_session_id {
        ...
        let created = sandbox
            .session(SessionOptions {
                ...
                name: Some(format!("chevalier-agent-tools-{suffix}")),
                image: Some(image),
                auto_start: true,
...
2. Start an async job that emits progress ticks and writes 'async-done' to /tmp/chevalier_async_job.txt.
3. Poll job status until it is completed.
4. Read /tmp/chevalier_async_job.txt.
