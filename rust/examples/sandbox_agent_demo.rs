//! Live sandbox demo using chevalier-sandbox from rust.
//!
//! Default mode is local managed daemon.
//! Set `CHEVALIER_SANDBOX_MODE=remote` to use an external daemon endpoint.
...
async fn run_exec(session: &Session, command: &str) -> Result<i32, Box<dyn std::error::Error>> {
    ...
    let exec_ready_timeout_secs = std::env::var("CHEVALIER_EXEC_READY_TIMEOUT_SECS")
        .ok()
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let endpoint = std::env::var("CHEVALIER_SANDBOX_ENDPOINT")
    ...
    let sandbox = if mode == "remote" {
        ...
        let data_dir = std::env::var("CHEVALIER_SANDBOX_DATA_DIR")
            .unwrap_or_else(|_| "/tmp/chevalier-agent-demo-vmd".to_string());
    ...
    let (parent_session_id, parent, attached_parent) = if let Some(session_id) = attach_session_id {
        ...
        let created = sandbox
            .session(SessionOptions {
                ...
                name: Some(format!("chevalier-agent-demo-{suffix}")),
                image: Some(image),
                auto_start: true,
    ...
    let exit = run_exec(
        &parent,
        "echo '[parent] hello from rust'; uname -a; cat /etc/os-release | head -n 2",
    )
    .await?;
    if exit != 0 {
