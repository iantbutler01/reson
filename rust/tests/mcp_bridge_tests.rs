...
fn build_calculator_server() -> chevalier_mcp::server::McpServer {
    chevalier_mcp::server::McpServer::builder("test-calculator")
...
async fn start_ws_server(port: u16) -> tokio::task::JoinHandle<()> {
    ...
    tokio::spawn(async move {
        server
            .serve(chevalier_mcp::server::ServerTransport::WebSocket(addr))
            .await
...
async fn test_agent_server_single() {
    ...
    let client = chevalier_mcp::client::McpClient::websocket("ws://127.0.0.1:19204")
        .await
...
async fn test_agent_server_with_raw_tool() {
    ...
    let client = chevalier_mcp::client::McpClient::websocket("ws://127.0.0.1:19205")
        .await
        .expect("Failed to connect");
...
async fn test_server_with_ui_resource() {
    ...
    let client = chevalier_mcp::client::McpClient::websocket("ws://127.0.0.1:19220")
        .await
        .expect("Failed to connect");
...
async fn test_ui_resource_with_csp_and_permissions() {
    ...
    let client = chevalier_mcp::client::McpClient::websocket("ws://127.0.0.1:19221")
        .await
        .expect("Failed to connect");
...
async fn test_tool_returning_error_content() {
    ...
    let server = chevalier_mcp::server::McpServer::builder("error-server")
        .with_tool(
            "fail",
            "Always fails",
            json!({"type": "object"}),
...
async fn test_tool_with_no_parameters() {
    let server = chevalier_mcp::server::McpServer::builder("no-args-server")
        .with_tool(
            "ping",
            "Returns pong",
            json!({"type": "object"}),
...
async fn test_tool_returning_multiple_content_blocks() {
    let server = chevalier_mcp::server::McpServer::builder("multi-content-server")
        .with_tool(
            "multi",
...
async fn test_multiple_servers_without_namespace() {
    ...
    let server1 = chevalier_mcp::server::McpServer::builder("server-a")
        .with_tool(
            "tool_a",
            "Tool from server A",
            json!({"type": "object"}),
    ...
    let server2 = chevalier_mcp::server::McpServer::builder("server-b")
        .with_tool(
            "tool_b",
            "Tool from server B",
            json!({"type": "object"}),
