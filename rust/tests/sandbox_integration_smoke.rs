...
use chevalier_sandbox::{ForkOptions, Sandbox, SandboxConfig, SessionOptions};
...
async fn sandbox_smoke_session_and_fork_when_enabled() {
    ...
    let suffix = SystemTime::now()
        ...
        .expect("current time")
        .as_secs();
    let parent_session_id = format!("rust-smoke-parent-{suffix}");

    let parent = sandbox
        .session(SessionOptions {
            session_id: Some(parent_session_id.clone()),
            name: Some(format!("rust-parent-{suffix}")),
            image: Some(image),
            auto_start: true,
