...
This document specifies the Python bindings needed to replace the pure-Python chevalier implementation with Rust via PyO3. The goal is to run existing Python integration tests unchanged.
...
from chevalier import (
    ...
    # Types (re-exported from chevalier.types)
    ChatMessage,
    ChatRole,
    ToolCall,
    ToolResult,
...
chevalier = { path = ".." }  # Core Rust library
...
1. Build `py` with maturin
2. Install as `chevalier` package (replaces pure Python)
3. Run integration tests
4. Fix any compatibility issues
5. Remove pure Python `chevalier/` directory

---

## 10. Not Included (Separate Concerns)

- **DatabaseManager / ORM** (`chevalier/data/postgres/`) - Separate package
